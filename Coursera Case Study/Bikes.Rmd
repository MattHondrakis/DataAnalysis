...
-   **Where**: We learned which stations were preferable to certain users. It showed us that *casuals* preferred stations along the lake shore of Chicago. This reinforced the hypothesis that casuals used Cyclistic for different purposes, namely leisure.

-   **What**: What bike was used did not provide much information. There were differences in the two groups but there is no way to clarify how that difference could improve memberships. Unfortunately, Cyclistic provides no information on the differences between the different bikes. Casuals are the only ones that were found to use *docked* bikes over the past year but it is unclear how this may relate to membership conversion.
